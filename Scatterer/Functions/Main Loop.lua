...
function main_loop()
    ...
    --Run scatterer
    --ProjConfigs[FocusedProj].playing = true
    if(ProjConfigs[FocusedProj].start_on_play)then
        ProjConfigs[FocusedProj].playing = reaper.GetPlayStateEx(FocusedProj)&1 == 1
    end
    Scatter(ProjConfigs[FocusedProj])
