    ...
    if reaper.ImGui_BeginChild(ctx, 'GroupSelect', -FLTMIN, avail_y-line_size*3.5, true, reaper.ImGui_WindowFlags_NoScrollbar()) then
        ...
        for region_idx, region_table in ipairs(playlist) do
            ...
            if reaper.ImGui_BeginPopupContextItem(ctx) then
                ...
                current_region = region_name -- Used for transition creation
...
function SaveTransitions()
    table.save(transitions, reaper.GetResourcePath() .. "/ReaGoTo_TransitionsData.lua")
end

-- Load transitions from file
function LoadTransitions()
    local path = reaper.GetResourcePath() .. "/ReaGoTo_TransitionsData.lua"
    local loaded_transitions = table.load(path)
    if loaded_transitions then
...
function RenameRegionMarkerPopUp(playlist, k)
    ...
    transitions[current_region] = transitions[current_region] or {}

    reaper.ImGui_Separator(ctx)
    reaper.ImGui_Text(ctx, "Transition from " .. current_region .. " to ")
    reaper.ImGui_SameLine(ctx)

    -- Combo Box: Target Region
    reaper.ImGui_SetNextItemWidth(ctx, 100)
    if reaper.ImGui_BeginCombo(ctx, "##TargetRegion", selected_target_region or "Select a region", reaper.ImGui_ComboFlags_NoArrowButton()) then
        local num_markers, num_regions = reaper.CountProjectMarkers(FocusedProj)
        for i = 0, num_markers + num_regions - 1 do
            local retval, _, _, _, region_name = reaper.EnumProjectMarkers2(FocusedProj, i)
            if retval and region_name and region_name ~= "" and region_name ~= current_region then
                if reaper.ImGui_Selectable(ctx, region_name, selected_target_region == region_name) then
                    selected_target_region = region_name
                end
            end
        end
    ...
    -- If there's a selected region
    if selected_target_region then
        -- Existing transitions between regions
        transitions[current_region][selected_target_region] = transitions[current_region][selected_target_region] or {}

        -- Show transition clips
        for i = #transitions[current_region][selected_target_region], 1, -1 do
            reaper.ImGui_PushID(ctx, i)

            -- Check bc sometimes initialization is string, we want table
            if type(transitions[current_region][selected_target_region][i]) ~= "table" then
                transitions[current_region][selected_target_region][i] = {track = "Select a track", note = "C", octave = "3"}
            end

            -- ComboBox: Select a track
            local selected_track = transitions[current_region][selected_target_region][i].track or "Select a track"
            reaper.ImGui_SetNextItemWidth(ctx, 150)
            if reaper.ImGui_BeginCombo(ctx, "##TrackSelect" .. i, selected_track, reaper.ImGui_ComboFlags_NoArrowButton()) then
                ...
                for _, track_name in ipairs(tracks) do
                    if reaper.ImGui_Selectable(ctx, track_name, selected_track == track_name) then
                        transitions[current_region][selected_target_region][i] = transitions[current_region][selected_target_region][i] or {}
                        transitions[current_region][selected_target_region][i].track = track_name
                        SaveTransitions()
                    end
                end
                reaper.ImGui_EndCombo(ctx)
            ...
            local octaves = {"-2", "-1", "0", "1", "2", "3", "4", "5", "6", "7", "8"}

            -- ComboBox: Select MIDI note
            reaper.ImGui_SameLine(ctx)
            reaper.ImGui_SetNextItemWidth(ctx, 20)
            local selected_note = transitions[current_region][selected_target_region][i].note or "C"
            if reaper.ImGui_BeginCombo(ctx, "##NoteSelect" .. i, selected_note, reaper.ImGui_ComboFlags_NoArrowButton()) then
                for _, note in ipairs(notes) do
                    if reaper.ImGui_Selectable(ctx, note, selected_note == note) then
                        transitions[current_region][selected_target_region][i] = transitions[current_region][selected_target_region][i] or {}
                        transitions[current_region][selected_target_region][i].note = note
                        SaveTransitions()
                    end
                end
                reaper.ImGui_EndCombo(ctx)
            ...
            -- ComboBox: Select MIDI note octave
            reaper.ImGui_SameLine(ctx)
            reaper.ImGui_SetNextItemWidth(ctx, 20)
            local selected_octave = transitions[current_region][selected_target_region][i].octave or "3"
            if reaper.ImGui_BeginCombo(ctx, "##OctaveSelect" .. i, selected_octave, reaper.ImGui_ComboFlags_NoArrowButton()) then
                for _, octave in ipairs(octaves) do
                    if reaper.ImGui_Selectable(ctx, octave, selected_octave == octave) then
                        transitions[current_region][selected_target_region][i] = transitions[current_region][selected_target_region][i] or {}
                        transitions[current_region][selected_target_region][i].octave = octave
                        SaveTransitions()
                    end
                end
                reaper.ImGui_EndCombo(ctx)
            ...
            if reaper.ImGui_Button(ctx, "X") then
                table.remove(transitions[current_region][selected_target_region], i)
                SaveTransitions()
            end
            ...
            if reaper.ImGui_Button(ctx, "+") then
                table.insert(transitions[current_region][selected_target_region], "")
                SaveTransitions()
            end
    end
